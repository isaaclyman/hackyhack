...
- `"TYPE 10"`: text appears one character at a time, as though someone were typing it, with a default 10 millisecond delay between characters.
...
Lets you set a condition for a set of commands. Most operators take a value (variable or quoted text) on both sides. Available operators are:
...
Creates or sets a variable named `$varname` to contain the text `value`. All variables must start with a dollar sign `$`.
...
- `RECTANGLE` is the shape that should be drawn. Options are:
  ...
  - `RECTANGLE BLANK` is a rectangle with no visible border.
